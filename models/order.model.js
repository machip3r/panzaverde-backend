  ...
  connection.execute(
    ...
    (error, orders) => {
      if (!data.hasOwnProperty('count'))
        return callback(error, { max_pages: 1, page: 1, records: orders.length, orders });
      connection.execute(
        "SELECT COUNT(id_order) as max_pages FROM pvOrder LIMIT 1",
        (error, rows) => {
          const max_pages = Math.round(rows[0].max_pages / data.count);
          return callback(error, { max_pages, page, records: orders.length, orders });
        }
      );
    }
  );
...
orderModel.findByStatus = (order, callback) => {
  let sql = 'SELECT * FROM pvOrder WHERE o_status LIKE :o_status ORDER BY o_date DESC';
  let page;
  if (data.hasOwnProperty('offset') && data.hasOwnProperty('count')) {
    sql += ' LIMIT :offset, :count';
    page = parseInt(data.offset);
