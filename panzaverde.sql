...
/* USER */
CREATE TABLE IF NOT EXISTS pvUser(
	id_user INT UNSIGNED AUTO_INCREMENT,
    u_name VARCHAR(50) NOT NULL,
    u_password VARCHAR(200) NOT NULL,
    u_type ENUM('Admin','Cooker','Driver','Other') NOT NULL,
    PRIMARY KEY (id_user)
);

/* INVENTORY */
CREATE TABLE IF NOT EXISTS pvProduct(
...
CREATE TABLE IF NOT EXISTS pvOrder(
...
CREATE TABLE IF NOT EXISTS pvOrderProduct(
    ...
    CONSTRAINT fk_order_product_product
        ...
        REFERENCES pvProduct (id_product)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
	CONSTRAINT fk_order_product_order
		FOREIGN KEY (id_order)
        REFERENCES pvOrder (id_order)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
...
/* ROUTES */
CREATE TABLE IF NOT EXISTS pvDriver(
	id_driver INT UNSIGNED AUTO_INCREMENT,
    d_name VARCHAR(80) NOT NULL,
    d_address VARCHAR(300) NOT NULL,
    d_phone VARCHAR(10) NOT NULL,
    d_license_plates VARCHAR(7) NULL,
    d_status CHAR(1) NOT NULL DEFAULT 'a',
    PRIMARY KEY (id_driver)
);

INSERT INTO pvDriver(d_name, d_address, d_phone, d_license_plates) VALUES ("Cesar Ortega", "Francisco Villa 203", "0987654321", "123-456");

CREATE TABLE IF NOT EXISTS pvRoute(
	id_route TINYINT UNSIGNED AUTO_INCREMENT,
    id_driver INT UNSIGNED NOT NULL,
    r_name VARCHAR(50) NOT NULL,
    r_color VARCHAR(30) NOT NULL,
    r_center_location VARCHAR(300) NOT NULL,
    CONSTRAINT fk_route_driver
		FOREIGN KEY (id_driver)
        REFERENCES pvDriver (id_driver)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    PRIMARY KEY (id_route)
);

INSERT INTO pvRoute (id_driver, r_name, r_color, r_center_location) VALUES (1, "Avenida Leon", "Azul", "Centro");

/* MEAL */
CREATE TABLE IF NOT EXISTS pvSocialNetwork(
...
CREATE TABLE IF NOT EXISTS pvClient(
    ...
    CONSTRAINT fk_client_social_network
		FOREIGN KEY (id_social_network)
        REFERENCES pvSocialNetwork (id_social_network)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    PRIMARY KEY (id_client)
);

CREATE TABLE IF NOT EXISTS pvSubscription(
    ...
    CONSTRAINT fk_subscription_client
        ...
        REFERENCES pvClient (id_client)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
	CONSTRAINT fk_subscription_route
		FOREIGN KEY (id_route)
        REFERENCES pvRoute (id_route)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    PRIMARY KEY (id_subscription)
);

CREATE TABLE IF NOT EXISTS pvDay(
...
CREATE TABLE IF NOT EXISTS pvTime(
...
CREATE TABLE IF NOT EXISTS pvMealType(
...
CREATE TABLE IF NOT EXISTS pvPlan(
    ...
    CONSTRAINT fk_plan_subscription
        ...
        REFERENCES pvSubscription (id_subscription)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
	CONSTRAINT fk_plan_day
		FOREIGN KEY (id_day)
        REFERENCES pvDay (id_day)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
	CONSTRAINT fk_plan_meal_type
		FOREIGN KEY (id_meal_type)
        REFERENCES pvMealType (id_meal_type)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
	CONSTRAINT fk_plan_time
		FOREIGN KEY (id_time)
        REFERENCES pvTime (id_time)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    PRIMARY KEY (id_subscription, id_day, id_meal_type, id_time)
);

/* RECIPES */
CREATE TABLE IF NOT EXISTS pvRecipe(
...
CREATE TABLE IF NOT EXISTS pvIngredient(
...
CREATE TABLE IF NOT EXISTS pvRecipeIngredient(
    ...
    CONSTRAINT fk_recipe_ingredient_recipe
        ...
        REFERENCES pvRecipe (id_recipe)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
	CONSTRAINT fk_recipe_ingredient_ingredient
		FOREIGN KEY (id_ingredient)
        REFERENCES pvIngredient (id_ingredient)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
